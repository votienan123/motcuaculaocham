...
@Entity
@Table(name = "giave2ngay1dem")
public class GiaVe2Ngay1Dem extends Model<GiaVe2Ngay1Dem> {
	...
	@Command
	public void saveGiaVe2Ngay1Dem(@BindingParam("list") final Object listObject, @BindingParam("attr") final String attr,
	...
	public void deleteTrangThaiConfirmAndNotify(final @BindingParam("notify") Object beanObject,
		...
		JPAQuery<DatVe> datVeQuery = find(DatVe.class).where(QDatVe.datVe.giaVe2Ngay1Dem.eq(this));
	...
	public AbstractValidator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(final ValidationContext ctx) {
				if (ten != null) {
					if (!ten.isEmpty()) {
						JPAQuery<GiaVe2Ngay1Dem> q = find(GiaVe2Ngay1Dem.class)
								.where(QGiaVe2Ngay1Dem.giaVe2Ngay1Dem.ten.trim().eq(ten.trim()));
						if (!noId()) {
							q.where(QGiaVe2Ngay1Dem.giaVe2Ngay1Dem.id.ne(getId()));
						}
						if (q.fetchCount() > 0) {
							addInvalidMessage(ctx, "lblErrTenLoaiTour", "Tên loại giá vé đang được sử dụng.");
						}
					}
				}				
			}
